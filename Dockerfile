...
# ============ build: compile the static PWA with npm ============
...
CMD ["sh", "-c", "while true; do echo '[updater] refreshing data'; node scripts/update.mjs || echo '[updater] run failed; retrying next cycle'; sleep \"$UPDATE_INTERVAL\"; done"]
...
ENV DATA_SOURCE=remote \
    REMOTE_DATA_HOST=julianchun.github.io/2026worldcup-dashboard
EXPOSE 80
