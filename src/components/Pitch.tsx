...
export default function Pitch({
  ...
  return (
    ...
    >
      ...
      <TeamLabel side="away" cy={4.6} textY={6} name={awayName} tactics={away?.tactics} flag={awayFlag} />
      ...
      {awayPlaced.map((pl) => (
        <PlayerLabels key={pl.p.id} pl={pl} side="away" off={subOff?.[pl.p.id]} goals={goals?.[pl.p.id]} />
      ))}
      {homePlaced.map((pl) => (
        <PlayerLabels key={pl.p.id} pl={pl} side="home" off={subOff?.[pl.p.id]} goals={goals?.[pl.p.id]} />
      ))}
